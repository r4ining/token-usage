...
**不单独计费（use_cache_price = 0，默认）：**

费用 = (提示词 token 数 × 输入价格 + 补全 token 数 × 输出价格) / 1,000,000

**缓存单独计费（use_cache_price = 1）：**

非缓存 token 数 = max(提示词 token 数 − 缓存 token 数, 0)

缓存价格 = 缓存价格配置 ≠ 0 ? 缓存价格配置 : 输入价格

费用 = (非缓存 token 数 × 输入价格 + 缓存 token 数 × 缓存价格 + 补全 token 数 × 输出价格) / 1,000,000

> 两者区别：不单独计费时所有提示词 token 统一按输入价格收费；单独计费时从提示词 token 中剥离出缓存 token，非缓存部分按输入价格、缓存部分按缓存价格分别计费，避免重复收费且支持差异化定价。
